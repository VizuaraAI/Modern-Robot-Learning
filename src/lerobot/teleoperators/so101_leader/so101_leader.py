...
class SO101Leader(Teleoperator):
    ...
    def __init__(self, config: SO101LeaderConfig):
        ...
        # WORKAROUND: Motor 5 has bit 15 stuck on - patch decode AND encode to treat it as unsigned
        ...
        def patched_decode(data_name, ids_values):
            ...
            # If motor 5 position/offset is negative, flip it to positive (bit 15 stuck)
            if 5 in result and result[5] < 0:
                if data_name in ["Present_Position", "Homing_Offset"]:
                    result[5] = -result[5]  # Flip sign: -2197 becomes +2197
            return result
